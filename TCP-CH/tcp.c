            ...
            while (1) {
                ...
                if (re_err == ERR_OK) {
                    do {
                        ...
                        event.buff = data;
                        if (xQueueSend(buff_queue, &event, pdMS_TO_TICKS(10)) == pdTRUE)
...
void tcp_server_subtask(void *Parameter) // 从串口接收发到TCP tcp_sender_from_uart_queue
{
    ...
    while (task_flag) {
        events event;
        if (xQueuePeek(rx_buff_queue, &event, portMAX_DELAY) == pdTRUE) {
...
TcpTaskHandle_t *TcpTaskCareate(void *Parameter) {
    ...
    char pcName[18];
    switch (Param->mode) {
        case SEND:sprintf(pcName, "Tcp%s%d", txname, TcpHandle.TaskNum);
            xTaskCreatePinnedToCore(tcp_send_server,
                                    (const char *const) pcName,
                                    5120,
                                    Parameter,
                                    14,
                                    &TCP_TASK_HANDLE[TcpHandle.TaskNum],
                                    0);
            TcpHandle.TaskHandle[TcpHandle.TaskNum] = &TCP_TASK_HANDLE[TcpHandle.TaskNum];
            TcpHandle.TaskNum++;
            break;
        case RECEIVE:sprintf(pcName, "Tcp%s%d", rxname, TcpHandle.TaskNum);
            ...
            TcpHandle.TaskNum++;
            break;
        case ALL:sprintf(pcName, "Tcp%s%d", allname, TcpHandle.TaskNum);
            xTaskCreatePinnedToCore(tcp_server,
                                    (const char *const) pcName,
                                    5120,
                                    Parameter,
                                    14,
...
uint8_t TcpTaskAllDelete(TcpTaskHandle_t *TcpHandle) {
    for (int i = 0; i < TcpHandle->TaskNum; i++) {
        if ((TcpHandle->TaskHandle[i]) != NULL) {
            vTaskDelete((TaskHandle_t) (TcpHandle->TaskHandle[i]));
        } else {
            break;
        }
    }
