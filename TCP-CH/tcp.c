...
int TcpHandle_FatherTask_current = 0;
...
TcpTaskHandle_t *TcpTaskCreate(void *Parameter) {
    ...
    switch (Param->mode) {
        case SEND:sprintf(pcName, "Tcp%s%d", txname, TcpHandle_FatherTask_current);
            ...
            TCP_TASK_HANDLE[TcpHandle_FatherTask_current].FatherTaskPort = Param->port;
...
uint8_t TcpTaskAllDelete(TcpTaskHandle_t *TCP_TASK_HANDLE_delete) {
    int need_to_delete_taskcount = 0;
...
void tcp_server(void *Parameter) {
    ...
    /* Create a new connection identifier. */
    create_tcp_server(Param->port, &conn_All);
